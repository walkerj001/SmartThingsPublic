...
metadata {
	...
	definition(name: "My Ecobee Device", author: "Yves Racine", namespace: "yracine", mnmn: "SmartThings", vid: "SmartThings-smartthings-Z-Wave_Thermostat") {
		capability "Sensor"
		capability "Actuator"
...
void setBacklightOffTime (value) {
	...
	if (!value.isNumber()) {
		traceEvent(settings.logFilter,"setBacklightOffTime>value $value is invalid, must be an integer value in seconds",settings.trace, get_LOG_ERROR(),true)
		return    
	}    
...
void setBacklightSleepIntensity (value) {
	if ((!value.isNumber())  || (value <0 || value>10)) {
		traceEvent(settings.logFilter,"setBacklightSleepIntensity>value $value is invalid, must be an integer between 0 and 10",settings.trace, get_LOG_ERROR(),true)
		return    
	}    
...
void setBacklightOnIntensity (value) {
	if ((!value.isNumber())  || (value <0 || value>10)) {
		traceEvent(settings.logFilter,"setBacklightOnIntensity>value $value is invalid, must be an integer between 0 and 10",settings.trace,get_LOG_ERROR(), true)
		return    
	}    
...
void poll() {
	...
	if (isTokenExpired()) {
		...
		if (!refresh_tokens()) {
			traceEvent(settings.logFilter,"poll>not able to renew the refresh token", settings.trace, get_LOG_WARN())         
		} else {
...
void setThermostatSettings(thermostatId,tstatSettings = []) {
	...
	if (((statusCode) && (!exceptionCheck?.contains("Bad Request"))) || 
		...
		if (!(interval=get_exception_interval_delay( state?.retriesSettingsCounter))) {  
			traceEvent(settings.logFilter,"setThermostatSettings>error max retries, counter=${state?.retriesSettingsCounter}, exiting", true, get_LOG_ERROR())
			reset_replay_data('Settings')                
			state?.retriesSettingsCounter=0
			return        
		}        
...
void setHoldWithHoldType(thermostatId, coolingSetPoint, heatingSetPoint, fanMode,
	...
	if ((holdType == 'dateTime') && (startDate==null || startDate=="")) {
		traceEvent(settings.logFilter,"setHoldWithHoldType>error, the holdType $holdType requires startDate param to bet set for ${thermostatId}",true, get_LOG_ERROR())
		return    
	} 
	if ((holdType == 'holdHours') && ((holdHours==null  || ((!holdHours.isNumber())) || holdHours <=0))) {
		traceEvent(settings.logFilter,"setHoldWithHoldType>error, the holdType $holdType requires holdHours param to bet set with an integer value greater than 0 for ${thermostatId}",true, get_LOG_ERROR())
		return    
	} 
...
void setHoldExtraParams(thermostatId, coolingSetPoint, heatingSetPoint, fanMode,
	...
	if (((statusCode) && (!exceptionCheck?.contains("Bad Request"))) || 
		...
		if (!(interval=get_exception_interval_delay(state?.retriesHoldCounter))) {  
			traceEvent(settings.logFilter,"setHold>error max retries, counter=${state?.retriesHoldCounter}, exiting", true, get_LOG_ERROR())
			reset_replay_data('Hold')                
			state?.retriesHoldCounter=0
			return        
		}        
...
void setHoldReplay() {
	...
	for (int i=1; (i<= get_MAX_SETTER_RETRIES()); i++) {
		...
		traceEvent(settings.logFilter,"setHoldReplay>about to recall setHold for thermostatId=$id,retries counter=$i" +
			",coolingSetpoint=$coolingSetpoint, heatingSetpoint=$heatingSetpoint,fanMode=$fanMode,tstatSettings=$tstatSettings,extraHoldParams=$extraHoldParams", true, get_LOG_INFO())
...
void createVacation(thermostatId, vacationName, targetCoolTemp, targetHeatTemp,
	...
	if (((statusCode) && (!exceptionCheck?.contains("Bad Request"))) || 
		...
		if (!(interval=get_exception_interval_delay((state?.retriesCreateVacationCounter)))) {  
			traceEvent(settings.logFilter,"createVacation>error max retries, counter=${state?.retriesCreateVacationCounter}, exiting", true, get_LOG_ERROR())
			reset_replay_data('CrVacation')                
			state?.retriesCreateVacationCounter=0
			return        
		}        
		...
		traceEvent(settings.logFilter,"createVacation>about to call createVacationReplay,retries counter=${state?.retriesCreateVacationCounter}", true, get_LOG_INFO())
...
void createVacationReplay() {
	...
	for (int i=1; (i<= get_MAX_SETTER_RETRIES()); i++) {
		...
		traceEvent(settings.logFilter,"createVacationReplay>about to call createVacation,retries counter=$i" +
			",vacationName=$vacationName,targetCoolTemp=$targetCoolTemp,targetHeatTemp=targetHeatTemp", true, get_LOG_INFO())
...
void deleteVacation(thermostatId, vacationName) {
	...
	api('deleteVacation', bodyReq) {resp ->
		...
		if (!statusCode) {
			/* when success, reset the exception counter */
			state.exceptionCount=0
			if ((data."replayDelVacatationId${state?.retriesDeleteVacationCounter}" == null) ||         // reset the counter if last entry is null
	...
	if (((statusCode) && (!exceptionCheck?.contains("Bad Request"))) || 
		...
		if (!(interval=get_exception_interval_delay(state?.retriesDeleteVacationCounter))) {  
			traceEvent(settings.logFilter,"deleteVacation>error max retries, counter=${state?.retriesDeleteVacationCounter}, exiting", true, get_LOG_ERROR())
			reset_replay_data('DelVacation')                
			state?.retriesDeleteVacationCounter=0
			return        
		}        
		...
		traceEvent(settings.logFilter,"deleteVacation>about to call createVacationReplay,retries counter=${state?.retriesDeleteVacationCounter}", true, get_LOG_INFO())
...
void deleteVacationReplay() {
	...
	for (int i=1; (i<= get_MAX_SETTER_RETRIES()); i++) {
		...
		traceEvent(settings.logFilter,"deleteVacationReplay>about to call deleteVacation,retries counter=$i" +
			",vacationName=$vacationName", true, get_LOG_INFO())
...
void resumeProgram(thermostatId=settings.thermostatId, resumeAllFlag=true) {  
	...
	if (exceptionCheck?.contains("exception")) {
		sendEvent(name: "verboseTrace", value: "", displayed:(settings.trace?:false)) // reset verboseTrace            
		traceEvent(settings.logFilter,"resumeProgram>exception=${exceptionCheck}", true, get_LOG_ERROR())
		statusCode=(statusCode)?:3 //internal error            
	}                
	...
	if (((statusCode) && (!exceptionCheck?.contains("Bad Request"))) || 
		...
		if (!(interval=get_exception_interval_delay(retriesResumeCounter))) {  
			traceEvent(settings.logFilter,"resumeProgram>error max retries, counter=${state?.retriesResumeCounter}, exiting", true, get_LOG_ERROR())
			state?.retriesResumeCounter=0					            
			reset_replay_data('Resume')                
			return        
		}        
		...
		traceEvent(settings.logFilter,"resumeProgram>about to call resumeProgramReplay,retries counter=${state?.retriesResumeCounter}", true, get_LOG_INFO())
...
void setClimateWithHoldType(thermostatId, climateName,
	...
	if ((holdType == 'dateTime') && (startDate==null || startDate=="")) {
		traceEvent(settings.logFilter,"setClimateWithHoldType>error, the holdType $holdType requires startDate param to bet set for ${thermostatId}",true, get_LOG_ERROR())
		return    
	} 
	if ((holdType == 'holdHours') && ((holdHours==null  || ((!holdHours.isNumber())) || holdHours <=0))) {
		traceEvent(settings.logFilter,"setClimateWithHoldType>error, the holdType $holdType requires holdHours param to bet set with an integer value greater than 0 for ${thermostatId}",true, get_LOG_ERROR())
		return    
	} 
	...
	for (i in 0..data.thermostatList.size() - 1) {
		...
		if (((statusCode) && (!exceptionCheck?.contains("Bad Request"))) || 
			...
			if (!(interval=get_exception_interval_delay(state?.retriesClimateCounter))) {  
				traceEvent(settings.logFilter,"setClimate>error max retries, counter=${state?.retriesClimateCounter}, exiting", true, get_LOG_ERROR())
				state?.retriesClimateCounter=0
				reset_replay_data('Climate')                
				return        
			}        
			...
			traceEvent(settings.logFilter,"setClimate>about to call setClimateReplay,retries counter=${state?.retriesClimateCounter}", true, get_LOG_INFO())
...
void setClimateReplay() {
	for (int i=1; (i<= get_MAX_SETTER_RETRIES()); i++) {
		...
		traceEvent(settings.logFilter,"setClimateReplay>about to recall setClimate for thermostatId=$id,retries counter=${state?.retriesClimateCounter}" +
			",climateName=$climateName, paramsMap=$paramsMap", true, get_LOG_INFO())
...
void updateAudio(thermostatId, playbackVolume, microphoneEnabled=null, soundAlertVolume=null,soundTickVolume=null) {
	...
	if ((playbackVolume != null) && ((!playbackVolume.isNumber()) || (playbackVolume < 0 || playbackVolume > 100))) {
		traceEvent(settings.logFilter,"updateAudio>playback volume is invalid ($playbackVolume), volume must be between 0 and 100", settings.trace, get_LOG_ERROR())	
		return    
	}    
	if ((microphoneEnabled != null) && (!microphoneEnabled in ['true','false'])) {
		traceEvent(settings.logFilter,"updateAudio>microphone enabled must be a boolean value ($microphoneEnabled)", settings.trace, get_LOG_ERROR())	
		return    
	}
	if ((soundAlertVolume != null) && ((!soundAlertVolume.isNumber()) || (soundAlertVolume < 0 || soundAlertVolume > 10))) {
		traceEvent(settings.logFilter,"updateAudio>soundAlertVolume  is invalid ($soundAlertVolume), soundAlertVolume must be between 0 and 10", settings.trace, get_LOG_ERROR())	
		return    
	}    
	if ((soundTickVolume != null) && ((!soundTickVolume.isNumber()) || (soundTickVolume < 0 || soundTickVolume > 10))) {
		traceEvent(settings.logFilter,"updateAudio>soundTickVolume  is invalid ($soundTickVolume), soundAlertVolume must be between 0 and 10", settings.trace, get_LOG_ERROR())	
		return    
	}    
...
//	def args_encoded = java.net.URLEncoder.encode(bodyReq.toString(), "UTF-8")
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		...
		api('updateAudio', bodyReq) {resp ->
			...
			def message = resp?.data?.status?.message
			if ((resp?.status==TOKEN_EXPIRED) || ((resp?.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH))) {
				traceEvent(settings.logFilter,"updateAudio>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()",settings.trace, get_LOG_WARN())
				refresh_tokens()     
			}
			exceptionCheck=device.currentValue("verboseTrace")
			if (!statusCode) {
				...
				state.exceptionCount = state.exceptionCount +1     
				traceEvent(settings.logFilter,"updateAudio>error ${statusCode.toString()},message=${message} for thermostatId =${thermostatId},plugName =${plugName}", true, get_LOG_ERROR())
			} /* end if statusCode */
		} /* end api call */               
		if (exceptionCheck?.contains("exception")) {
			sendEvent(name: "verboseTrace", value: "", displayed:(settings.trace?:false)) // reset verboseTrace            
			traceEvent(settings.logFilter,"updateAudio>exception=${exceptionCheck}, loop counter=$j", true, get_LOG_ERROR())
			statusCode=(statusCode)?:3 //internal error            
		}                
	} /* end while */
...
void getRemoteSensorUpdate(thermostatId=settings.thermostatId) {
	...
	if ((statusCode) || (exceptionCheck?.contains("ConnectTimeoutException"))) {
		...
		if (!(interval=get_exception_interval_delay( state?.retriesCounter, "GETTER"))) {  
			traceEvent(settings.logFilter,"getRemoteSensorUpdate>error max retries, counter=${state.retriesCounter}, exiting", true, get_LOG_ERROR())
			return        
		}        
		data?.replayRemoteSensorId=thermostatId
		traceEvent(settings.logFilter,"getRemoteSensorUpdate>about to call getRemoteSensorsUpdateReplay,retries counter=${state.retriesCounter}", true, get_LOG_INFO())
...
void getThermostatInfo(thermostatId=settings.thermostatId) {
	...
	if (exceptionCheck?.contains("exception")) {
		sendEvent(name: "verboseTrace", value: "", displayed:(settings.trace?:false)) // reset verboseTrace            
		traceEvent(settings.logFilter,"getThermostatInfo> exception=${exceptionCheck}",true, get_LOG_ERROR())
		statusCode=(statusCode)?:3 //internal error            
	}                
	if (((statusCode) && (!exceptionCheck?.contains("Bad Request"))) || 
		(exceptionCheck?.contains("ConnectTimeoutException"))) {
		state?.retriesCounter=(state?.retriesCounter?:0)+1            
		if (!(interval=get_exception_interval_delay( state?.retriesCounter, "GETTER"))) {  
			traceEvent(settings.logFilter,"getThermostatInfo>error max retries, counter=${state.retriesCounter}, exiting", true, get_LOG_ERROR())
			state?.retriesCounter=0            
			return        
		}        
...
void getThermostatSummary(tstatType) {
	...
	if (exceptionCheck?.contains("exception")) {
		sendEvent(name: "verboseTrace", value: "", displayed:(settings.trace?:false)) // reset verboseTrace            
		traceEvent(settings.logFilter,"getThermostatSummary>exception=${exceptionCheck}",true, get_LOG_ERROR())
		statusCode=(statusCode)?:3 //internal error            
	}                
	...
	if (((statusCode) && (!exceptionCheck?.contains("Bad Request"))) || 
		...
		state?.retriesCounter=(state?.retriesCounter?:0)+1            
		if (!(interval=get_exception_interval_delay( state?.retriesCounter, "GETTER"))) {  
			traceEvent(settings.logFilter,"getThermostatSummary>error max retries, counter=${state.retriesCounter}, exiting", true, get_LOG_ERROR())
			state?.retriesCounter=0            
			return        
		}        
		data?.replayEcobeeType=tstatType
		traceEvent(settings.logFilter,"getThermostatSummary>about to call getThermostatSummaryReplay,retries counter=${state.retriesCounter}", true, get_LOG_INFO())
...
private def refreshLocalAuthToken() {	
	...
	httpPost(refreshParams) { resp ->
		if (resp?.status == REFRESH_SUCCESS_CODE) {
			...
			traceEvent(settings.logFilter,"refreshLocalAuthToken>refresh failed ${resp.status} : ${resp?.status}",true, get_LOG_ERROR())
...
def retrieveDataForGraph() {
	...
	if ( runtimeStatsTable == null || runtimeAvgWeeklyStatsTable == null || runtimeAvgMonthlyStatsTable == null) {
		...
		if (mode=='cool') {
			...
			def heatStatsData = device.statesSince("compHeat1RuntimeDaily", startOfWeek, [max:10])
			def heatWeeklyStatsData = device.statesSince("compHeat1RuntimeAvgWeekly", startOfWeek, [max:10])
			def heatMonthlyStatsData = device.statesSince("compHeat1RuntimeAvgMonthly", startOfWeek, [max:10])
			def hasHeatPumpFlag = (device.currentValue("hasHeatPump")) ?: 'false'
			if ((hasHeatPumpFlag=='false') || (!heatMonthlyStatsData) || (heatMonthlyStatsData[0]==0)) {
				heatStatsData = device.statesSince("auxHeat1RuntimeDaily", startOfWeek, [max:10])
				heatWeeklyStatsData = device.statesSince("auxHeat1RuntimeAvgWeekly", startOfWeek, [max:10])
				heatMonthlyStatsData = device.statesSince("auxHeat1RuntimeAvgMonthly", startOfWeek, [max:10])
			}			            
